defmodule CBDashboard.Components.UI do
  @moduledoc """
  Shared presentation primitives for the System Observability Dashboard.

  Every LiveView in `lib/louder/observability/live/` (and the
  BeliefContext component) renders through these helpers so we only
  describe each visual pattern once. Token names come from
  `CBDashboard.Layouts`'s `:root` block — see the comment
  header there for the badge / banner / overlay / kind / maxw groups.
  """
